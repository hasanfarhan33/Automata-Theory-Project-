...
public class DivideAndConquer extends Canvas {
	...
	public static ArrayList<Float> NextCityXhafl1 = new ArrayList<Float>();
	public static ArrayList<Float> NextCityYhafl1 = new ArrayList<Float>();
	...
	public static void Nearest_Neighbour_Window()
	{
		 ...
		 JFrame Main_Window = new JFrame("DivideAndConquer");
		 Canvas canvas = new DivideAndConquer();
	...
	public void paint(Graphics g)
    {
		...
		*/
		...
		for(int i = 0; i < NextCityXhafl1.size(); i++)
		{
			g.setColor(Color.RED);
			g.drawLine((int)(NextCityXhafl1.get(i) % 1000), (int)(NextCityYhafl1.get(i) % 1000), (int)(NextCityXhafl1.get(i+1) % 1000), (int)(NextCityYhafl1.get(i+1) % 1000));

			if(NextCityXhafl1.get(i+1) == NextCityXhafl1.get(NextCityXhafl1.size() - 1))
			{
				break;
			}
		...
		{
			g.setColor(Color.RED);
			g.drawLine((int)(xR_Coordinates.get(i) % 1000), (int)(yR_Coordinates.get(i) % 1000), (int)(xR_Coordinates.get(i+1) % 1000), (int)(yR_Coordinates.get(i+1) % 1000));
			g.drawLine((int)(xL_Coordinates.get(i) % 1000), (int)(yL_Coordinates.get(i) % 1000), (int)(xL_Coordinates.get(i+1) % 1000), (int)(yL_Coordinates.get(i+1) % 1000));

			if(xR_Coordinates.get(i+1) == xR_Coordinates.get(xR_Coordinates.size() - 1))
			{
				break;
			}
			if(xL_Coordinates.get(i+1) == xL_Coordinates.get(xL_Coordinates.size() - 1))
			{
				break;
			}
		...
		/*
		for(int i = 0; i < xL_Coordinates.size(); i++)
		{
	...
	public static void divide_Conquer_Nearest_Neighbour_Algorithm()
	{
		...
		for(int i= X_Coordinates.size()/2 ; i < X_Coordinates.size() ;i++){
		...
		for(int i= Y_Coordinates.size()/2 ; i <Y_Coordinates.size() ;i++){
		...
		if(NextCityX.isEmpty())
		{
			for(int i = 0; i < xL_Coordinates.size(); i++) //Calculate the shortest distance for the first city
			{
				...
				if(i == xL_Coordinates.size() - 1 && CurrentShortestDistances.isEmpty() == false)
				{
		...
		for(int i = 0; i < NextCityX.size(); i++)
		{
			for(int j = 0; j < xL_Coordinates.size(); j++)
			{
				...
				if(LastElementsRemoved == false)
				{
					...
					if(j == xL_Coordinates.size() - 1 && CurrentShortestDistances.isEmpty() == false)
					{
		...
		for (int i= 0 ; i<NextCityX.size();i++){
			NextCityXhafl1.add(NextCityX.get(i));
		}
		for (int i= 0 ; i<NextCityX.size();i++){
			NextCityYhafl1.add(NextCityY.get(i)); ;
		}
///////////////////////
		...
		if(NextCityX.isEmpty())
		{
			for(int i = 0; i < xR_Coordinates.size(); i++) //Calculate the shortest distance for the first city
			{
				...
				if(i == xR_Coordinates.size() - 1 && CurrentShortestDistances.isEmpty() == false)
				{
					ShortestDistances.add(CurrentShortestDistances.peek());
					NextCityX.add(CurrentShortestCoordinateX.peek());
					NextCityY.add(CurrentShortestCoordinateY.peek());
		...
		for(int i = 0; i < NextCityX.size(); i++)
		{
			for(int j = 0; j < xR_Coordinates.size(); j++)
			{
				...
				if(LastElementsRemoved == false)
				{
					...
					if(j == xR_Coordinates.size() - 1 && CurrentShortestDistances.isEmpty() == false)
					{
